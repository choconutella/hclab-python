...
class Validate():
  ...
  def no_repeat(self,cnt:int)->bool:
    """Check current number already successfully sent email"""

    sql = "select count(wa_tno) from sine_wa_log where wa_tno = :lno and wa_status = 'SENT'"
    params = {'lno' : self.lno}
    
    with self.engine.connect() as conn:
      record = conn.execute(sql, params).fetchone()

    return True if record[0] < cnt else False
  ...
  def authorise(self)->bool:
    ...
    return True if records is None else False
